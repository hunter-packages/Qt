...
void QAndroidCameraSession::applyViewfinderSettings(const QSize &captureSize, bool restartPreview)
{
    ...
    if (m_requestedViewfinderSettings.pixelFormat() != QVideoFrame::Format_Invalid) {
        ...
        if (f == AndroidCamera::Unknown || !m_camera->getSupportedPreviewFormats().contains(f))
    ...
    if (requestedFpsRange.min > 0 || requestedFpsRange.max > 0) {
        ...
        for (int i = supportedFpsRanges.size() - 1; i >= 0; --i) {
            const AndroidCamera::FpsRange it = supportedFpsRanges[i];
            if ((it.min == requestedFpsRange.min && it.max == requestedFpsRange.max)
                    || (requestedFpsRange.min <= 0 && requestedFpsRange.max == it.max)
                    || (requestedFpsRange.max <= 0 && requestedFpsRange.min == it.min)) {
                adjustedFps = it;
                found = true;
                break;
            }
...
void QAndroidCameraSession::applyImageSettings()
{
    ...
    if (!requestedResolution.isValid()) {
        ...
        if (m_requestedViewfinderSettings.resolution().isValid()) {
            ...
            for (int i = supportedResolutions.size() - 1; i >= 0; --i) {
                const QSize it = supportedResolutions[i];
